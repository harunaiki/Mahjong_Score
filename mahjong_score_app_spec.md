...
- バージョン: 1.4
- 作成日: 2026-07-18 / 改訂: 2026-07-19
- 改訂履歴 v1.4: ①局終了(和了・流局・チョンボ・複数和了)ごとに「清算確認」画面を挟んでから確定する方式に変更(6.3.5) ②流局のテンパイ選択でリーチ者を自動選択・変更不可に(6.4) ③プレイヤー情報の端末間同期を修正(listレスポンスにplayersを含める。15.2/15.4) ④GAS URL・合言葉の端末ごと再入力を解消(HTML内デフォルト設定+設定共有リンク。15.5)
...
- 「点数」セクション: アコーディオンリスト
  ...
  - **行タップで清算確認画面(6.3.5)へ遷移**(v1.4変更。即確定はしない)
- 確定時処理(清算確認画面の「確定」押下で実行): 点数授受+本場加算(設定値×本場数)+供託全回収+焼き鳥フラグ解除+連荘/親流れ判定(9章)+次局へ自動進行し対局画面へ戻る
- SHOULD: 確定直後、対局画面に授受サマリをトースト表示(例「伊藤栄 +12,300 / いっせい −12,300(本場600・供託1,000含む)」)

**6.3.5 清算確認画面(v1.4追加・添付UIスクリーンショット準拠)**
すべての局終了イベント(**和了・複数和了・流局・チョンボ**)は、点数を反映する前に必ずこの画面を表示し、「確定」押下ではじめてイベントを確定・次局へ進行する。

- ヘッダー: 左「<」戻る(前の入力画面へ戻り選び直せる)/中央タイトル「清算確認」
- 「点数変動」セクション: 参加者ごとのカードを縦に並べる
  - 左: プレイヤー名
  - 右上: 増減点(大きく)。正=青「+5,900 点」、負=赤「-3,900 点」、変動なし=「+0 点」(青)
  - 右下: 変動前→変動後の持ち点(グレー小)。例「25,000 → 21,100」
  - 増減点には**本場加算・供託・ノーテン罰符まで含めた最終値**を表示する(内訳は下の情報カードで示す)
- 情報カード(グレー背景。イベント種別に応じて表示):
  - 和了時「和了情報」: 和了者ごとに1行 — 左=和了者名、右=1行目「ツモ」or「ロン (放銃者名)」(グレー)、2行目「3翻30符」(太字)、3行目=基本支払表現(ロン「3,900点」/ツモ「1,000-2,000」/親ツモ「2,000∀」)
  - 「対局情報」: 供託「供託: 2本 → しんごが供託2本を獲得」、本場「1本場 → +300点(放銃者負担)」、連荘/親流れの予告(「親流れ → 東2局へ」)を該当時のみ表示
  - 流局時「流局情報」: テンパイ者名の列挙、ノーテン罰符の授受内訳、供託繰越本数、連荘/親流れ予告
  - チョンボ時「チョンボ情報」: 違反者名、罰則方式、同一局やり直しの旨
- 下部: 「確定」(青・大・固定)。確定でイベント記録→点数反映→次局へ。戻るで破棄せず入力画面へ
- 確定後の取り消しは従来通りUndo(6.6)で可能

**複数和了(ダブロン/トリプルロン)**
- 中央ボタン→「複数和了」: ①放銃者を1名選択 ②和了者を複数選択 ③和了者ごとに点数(翻・符)を順に入力 → **清算確認画面(6.3.5)** → 確定
- 供託: 設定「立直棒の受取者」に従う。頭ハネ=放銃者から反時計回りに最も近い和了者のみ獲得/全員=按分せず各自の宣言棒のみ回収し場の供託は頭ハネ者
- 本場加算: 頭ハネの和了者のみ受け取る
- 親が和了者に含まれれば連荘

**チョンボ**
- 中央ボタン→「チョンボ」: 違反者を1名選択 → **清算確認画面(6.3.5。チョンボ情報カード付き)** → 確定
...
- **リーチ宣言者は自動的に「テンパイ」選択済みで初期表示し、変更不可(ロック表示: 選択状態+リーチ棒アイコン、タップ無効)**とする(v1.4。リーチ=テンパイ確定のため)。
- 選択後「次へ」→ **清算確認画面(6.3.5。流局情報カード付き)** → 確定で罰符授受・局進行を反映(v1.4)。
...
interface Player { id: string; name: string; createdAt: number; updatedAt: number; archived?: boolean; }
...
- A-7: 和了・流局・チョンボ・複数和了のいずれも、確定前に清算確認画面(各人の増減点と変動前→後の持ち点、本場・供託込み)が表示され、「確定」で初めて点数が反映される。「<」で入力画面へ戻って選び直せる。
- A-8: リーチ宣言者がいる状態で流局を開くと、その者は「テンパイ」選択済み・変更不可で表示される。
...
- D-6: 端末Aで新規プレイヤーを登録(または改名)→端末Bの起動時に名簿へ反映される。プレイヤーマージも同様に反映される。
- D-7: localStorageを消去した端末(=初回起動相当)で開いても、APP_CONFIG により入力なしで同期が有効になる。設定共有リンクを開いた端末でも同様。
...
- **プレイヤー同期(v1.4で明確化・MUST)**: `action=list` のレスポンスに**全Player配列を含め**、対局と同じく `updatedAt` のlast-write-winsでローカルの `mj.players` にマージする。プレイヤーの新規作成・改名・アーカイブ・マージ(F-20)はすべて `upsertPlayers` で即時送信する。これにより端末Aで登録した名簿が端末Bの起動時に反映される。対局データ内の playerId に対応するプレイヤーがローカルに無い場合も、listで受け取ったPlayerから補完すること。
...
| GET `?action=list&token=..` | — | `{ok:true, games:[{id, updatedAt, deleted}], players:[Player]}`(playersは全件。v1.4) |
...
### 15.5 アプリ側の同期設定UI
- **同期設定の既定値をHTMLに内蔵する(v1.4・MUST)**: index.html の `<script>` 冒頭に設定定数を定義する。
  ```js
  // ==== config ====(デプロイ時にここへ自分のGAS URLと合言葉を記入する)
  const APP_CONFIG = { gasUrl: 'https://script.google.com/macros/s/xxxx/exec', token: 'aikotoba' };
  ```
  - 起動時、localStorageに同期設定が無ければ `APP_CONFIG` を自動採用する。個人サーバーに1枚だけ置く運用のため、**どの端末で開いても入力なしで同期が有効**になる。
  - 設定画面での手入力は上書き用として残す(localStorage値 > APP_CONFIG の優先順)。
- **設定共有リンク(v1.4・SHOULD)**: 「その他 > スプレッドシート連携」に「他端末用の設定リンクを表示」ボタンを追加。`(現在のURL)#setup=<Base64(JSON{gasUrl,token})>` を生成しQRコード/コピーで共有。リンクを開いた端末はハッシュを読み取って同期設定を保存し、ハッシュをURLから除去する。APP_CONFIGを書き換えずに配る場合の代替手段。
- 「その他 > データ」の「スプレッドシート連携」画面:
  - GASウェブアプリURL入力欄 / 合言葉入力欄(APP_CONFIG採用時はその値をプレースホルダ表示)/「接続テスト」ボタン(pingの結果表示)
